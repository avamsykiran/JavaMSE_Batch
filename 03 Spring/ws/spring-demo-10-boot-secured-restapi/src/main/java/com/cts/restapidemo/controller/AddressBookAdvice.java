...
public class AddressBookAdvice {
	...
	public ResponseEntity<ErrorMessage> handleUsernameNotFoundException(UsernameNotFoundException exp){
		logger.error(exp.getMessage(), exp);
		return new ResponseEntity<ErrorMessage>(new ErrorMessage(exp.getMessage()), HttpStatus.UNAUTHORIZED);
	}
